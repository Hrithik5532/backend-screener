...
class FinancialDataScraper:
    ...
    def scrape_company_data(self, company_slug: str, sections: List[str] = None) -> Dict:
        ...
        # if sections is None:
        sections = ['quarters', 'profit-loss', 'balance-sheet', 'ratios', 'cash-flow', 'shareholding']
...
def save_company_data_to_db(scraped_data: Dict, company_slug: str = None):
    ...
    try:
        ...
        company, created = Company.objects.get(
            slug=company_slug,
        )
        ...
        print(f"[+] 'Selected'company: {company.name or company.slug}")
        ...
        if key_ratios:
            try:
                ...
                if FinancialDataTable.objects.filter(company=company,table_type='ratios',data_type='consolidated',).exists():
                    ratios_table = FinancialDataTable.objects.get(company=company,table_type='ratios',data_type='consolidated',)
...
def get_metric_hierarchy(company_slug: str, table_type: str = None, data_type: str = None):
    ...
    """
    try:
        from Main.models import Company, KeyFinancialMetric
...
def get_metric_children(metric_id: int):
    ...
    try:
        from Main.models import KeyFinancialMetric
...
if __name__ == "__main__":
    ...
    def check_chrome_installation():
        """Check if Chrome is installed and provide installation instructions"""
        chrome_paths = [
            "/usr/bin/google-chrome",
            "/usr/bin/google-chrome-stable", 
            "/usr/bin/chromium-browser",
        ...
        print("="*60)
        print("Chrome browser is not installed. Please run ONE of these commands:")
        print()
        print("Option 1 - Install Google Chrome (Recommended):")
        ...
        print("  sudo apt install -y google-chrome-stable")
        print()
        print("Option 2 - Install Chromium (Alternative):")
        print("  sudo apt update")
        print("  sudo apt install -y chromium-browser")
        print()
        print("Option 3 - Quick install (if you have sudo access):")
        print("  curl -fsSL https://dl.google.com/linux/linux_signing_key.pub | sudo gpg --dearmor -o /usr/share/keyrings/google-chrome.gpg")
        print("  echo 'deb [arch=amd64 signed-by=/usr/share/keyrings/google-chrome.gpg] http://dl.google.com/linux/chrome/deb/ stable main' | sudo tee /etc/apt/sources.list.d/google-chrome.list")
    ...
    # Test the scraper
    print("\n" + "="*60)
    print("ENHANCED FINANCIAL DATA SCRAPER MODULE")
    print("="*60)
    print("✓ Captures ALL data including hidden columns")
    print("✓ Preserves original functionality (+ buttons, toggle)")
    print("✓ Saves to CSV for verification")
    print("✓ Enhanced database integration")
    print("✓ Proper Decimal handling for financial data")
    print("✓ Ubuntu/Linux compatibility")
    print("✓ Ready to be imported by API views")
    print("="*60 + "\n")
    
    # Check Chrome installation
    if not check_chrome_installation():
        print("[!] Please install Chrome first, then run the scraper again.")
        exit(1)
    
    print("[+] Chrome is installed. Starting scraper test...")
    
    try:
        test_companies = ["ULTRACEMCO"]
